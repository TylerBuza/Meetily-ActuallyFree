...
export function OptionalModelDownloads({ activeOnly = false }: { activeOnly?: boolean }) {
  ...
  return <section aria-label="Optional model downloads" className="w-full space-y-3 rounded-xl border border-[var(--af-border)] bg-[var(--af-panel)] p-4">
    <h3 className="font-semibold">Optional model downloads <span className="text-xs text-blue-500">Recommended</span></h3>
    <p className="text-xs text-[var(--af-text-2)]">Downloads continue while you use Meetily and automatically enable the model when complete. Keep the app open until downloads finish.</p>
    {visible.map(model => {
      ...
      return <div key={model} className="space-y-2">
        <div className="flex items-center justify-between gap-3">
          ...
          {job.status === 'ready' && job.enabled ? <span className="text-sm text-emerald-600">Enabled</span>
            : job.status === 'activating' ? <span role="status" className="text-sm">Enabling…</span>
            : job.status === 'downloading' ? <span role="status" className="text-sm">{Math.round(job.progress)}%</span>
            : <Button size="sm" variant="outline" onClick={() => startDownload(model)}>{job.status === 'activation-error' ? 'Retry activation' : job.status === 'ready' ? 'Enable' : job.status === 'error' ? 'Retry' : 'Download & enable'}</Button>}
        </div>
