...
mod support;
use support::{Error, Result, ModelConfig, extract_3d_f32};
...
impl Sortformer {
    ...
    pub fn with_config<P: AsRef<Path>>(
        ...
        let (chunk_len, fifo_len, spkcache_len, right_context, spkcache_update_period, sil_emb) = {
            ...
            let sil_emb = metadata
                .as_ref()
                .and_then(|m| m.custom("learnable_sil_emb").ok().flatten())
                .map(|raw| {
                    raw.split(',')
        ...
        let mel_basis =
            support::create_mel_filterbank(N_FFT, N_MELS, SAMPLE_RATE).mapv(to_bf16);
    ...
    fn extract_mel_features(&self, audio: &[f32]) -> Result<Array3<f32>> {
        ...
        let preemphasized = support::apply_preemphasis(audio, PREEMPH);
...
mod tests {
    ...
    #[test]
    fn overlapping_speakers_and_short_turns_retain_each_identity() {
        let mut probabilities = Array2::zeros((100, NUM_SPEAKERS));
        for t in 10..50 { probabilities[[t, 0]] = 0.9; }
        for t in 30..40 { probabilities[[t, 1]] = 0.9; }
        let segments = Sortformer::post_process(&DiarizationConfig::default(), &probabilities, 16000);
        assert_eq!(segments.len(), 2);
        assert_eq!((segments[0].speaker_id,segments[0].start,segments[0].end),(0,1600,8000));
        assert_eq!((segments[1].speaker_id,segments[1].start,segments[1].end),(1,4800,6400));
        assert!(Sortformer::post_process(&DiarizationConfig::default(), &Array2::zeros((100, NUM_SPEAKERS)),16000).is_empty());
    }
